...
class GameController {
    ...
    @Path("/user/{id}")
    @Transactional
    fun deleteGamesByUserId(@PathParam("id") userId: Long): Response {
        return try {
            val gamesForUser = gameRepository.findGamesByUserId(userId)

            gamesForUser.forEach { game ->
                val existingPlayersForThisGame = playersRepository.findPlayersByGameId(game.id)
                val existingDrawsForThisGame = drawsRepository.findByGameId(game.id)
